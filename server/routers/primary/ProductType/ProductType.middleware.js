...
class ProductTypeMiddleware extends BaseMiddleware {

  constructor(app) {
    super({
      app
    });
    this.permissions = require('./ProductType.policy').permissions;
    // this.$acl = app.acl;
    // this.$acl.addPolicy(require('./ProductType.policy').acl);
  }


  async default (req, res, next) {

    console.log('ProductType middleware;')
